...
TEST_CASE("test assign map", "[instantiation_map]") {
    ...
    REQUIRE_THROWS_AS(AmbientMap(std::filesystem::current_path() += path_to_map), std::runtime_error);
    
    path_to_map = "/../../tests/data/test_wrong_map.txt";
    REQUIRE_THROWS_AS(AmbientMap(std::filesystem::current_path() += path_to_map), std::runtime_error);
    
    path_to_map = "/../../tests/data/test_map.txt";
    REQUIRE_NOTHROW(AmbientMap(std::filesystem::current_path() += path_to_map));
}
...
TEST_CASE("test get number of rows and columns of a map", "[rows_and_columns_map]") {
    ...
    AmbientMap map {std::filesystem::current_path() += path_to_map};
