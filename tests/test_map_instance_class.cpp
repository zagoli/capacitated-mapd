...
TEST_CASE("test assign instance", "[instantiation_instance]") {
    std::filesystem::path path_to_map{"/../../tests/data/test_maps.txt"};
    std::filesystem::path path_to_instance{"/../../tests/data/test_instance.txt"};

    REQUIRE_THROWS_AS(AmbientMapInstance(std::filesystem::current_path() += path_to_instance, std::filesystem::current_path() += path_to_map), std::runtime_error);

    path_to_map = "/../../tests/data/test_map.txt";
    REQUIRE_NOTHROW(AmbientMapInstance(std::filesystem::current_path() += path_to_instance, std::filesystem::current_path() += path_to_map));
}

TEST_CASE("test get number of rows and columns of a instance", "[rows_and_columns_instance]") {
    
    std::filesystem::path path_to_map{"/../../tests/data/test_map.txt"};
    std::filesystem::path path_to_instance{"/../../tests/data/test_instance.txt"};
    AmbientMapInstance instance {std::filesystem::current_path() += path_to_instance, std::filesystem::current_path() += path_to_map};
